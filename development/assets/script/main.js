...
//console.log('👻', data);
let member=[];
let assistant=data.filter((element)=>{
	return element.assistant==true;
});
let newData=assistant.filter(element=>{
	return data.map(item=>{
		if(item.group==element.group && item.assistant==false){
			return member.push(`${item.id}, ${item.name}, ${item.assistant}`); 
		}
	})
});
for(let i in assistant){
	assistant[i]['member'] = member;
}
console.log(newData);
